...
export function DropZone({
    ...
    async (e: React.DragEvent) => {
      ...
      const walk = async (entry: FileSystemEntry, prefix: string): Promise<void> => {
        if (entry.isFile) {
          ...
          const entries: FileSystemEntry[] = await new Promise((res) => reader.readEntries((es) => res(es)));
  ...
  return (
    <div
      ...
      } ${over ? "border-blue-400/70 bg-blue-900/20" : "border-edge-strong/70 bg-raised/40 hover:border-edge-strong hover:bg-raised/60"}`}
    >
      <input
        ref={inputRef}
        type="file"
      ...
      <div className={`flex ${compact ? "flex-row items-center gap-3" : "flex-col items-center gap-3 text-center"}`}>
        <div
          className={`${compact ? "w-8 h-8" : "w-12 h-12"} rounded-full flex items-center justify-center shrink-0 ${
            over ? "bg-blue-500/20" : "bg-raised/60"
          }`}
        >
          {busy ? (
